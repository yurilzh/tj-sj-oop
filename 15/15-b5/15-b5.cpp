...
TString::TString(const TString& ts)
{
	if (!strcmp(ts.get_content(), "<NULL>")) {
		...
		if (!content)
			exit(-1);
		strncpy(content, ts.get_content(), len + 1);
	}
}

void TString::set(const TString& s)
{
	if (!strcmp(s.get_content(), "<NULL>")) {
		len = 0;
		content = NULL;
	}
	else {
		...
		if (!content)
			exit(-1);
		strncpy(content, s.get_content(), len + 1);
	}
}
