...
TString::TString(const TString& s)
{
	...
	this->len = s.len;
	strncpy(this->content, s.content, s.len);
...
TString::TString(const char* cs)
{
	if (cs) {
		...
		strncpy(this->content, cs, this->len);
	}
	else {
...
TString& TString::operator=(const TString& s)
{
	...
	this->content = new(nothrow) char[s.len + 1];
	this->len = s.len;
	strncpy(content, s.content, s.len);
	this->content[this->len] = '\0';
	return *this;
}
...
TString operator+(const TString& s1, const TString& s2)
{
	...
	strncpy(s3.content, s1.content, s1.len);
	strncpy(s3.content + s1.len, s2.content, s2.len);
	s3.content[s3.len] = '\0';
	return s3;
}
...
TString operator+(const char* s1, const TString& s2)
{
	if (s1) {
		...
		strncpy(s3.content, s1, s1len);
		strncpy(s3.content + s1len, s2.content, s2.len);
		s3.content[s3.len] = '\0';
		return s3;
	}
	else
...
TString operator+(const TString& s1, const char* s2)
{
	if (s2) {
		...
		strncpy(s3.content, s1.content, s1.len);
		strncpy(s3.content + s1.len, s2, s2len);
		s3.content[s3.len] = '\0';
		return s3;
	}
	else
...
TString operator+(const char c1, const TString& s2)
{
	TString s3(s2.len + 1);
	s3.content[0] = c1;
	strncpy(s3.content + 1, s2.content, s2.len);
	s3.content[s3.len] = '\0';
	return s3;
}
...
TString& TString::operator+=(const TString& s)
{
	...
	strncpy(newContent, this->content, this->len);
	strncpy(newContent + this->len, s.content, s.len);
	newContent[newlen] = '\0';
	delete[] this->content;
...
TString& TString::operator+=(const char* cs)
{
	if (cs) {
		...
		char* newContent = new(nothrow) char[newlen + 1];
		strncpy(newContent, this->content, this->len);
		strncpy(newContent + this->len, cs, slen);
		newContent[newlen] = '\0';
		delete[] this->content;
		this->content = newContent;
		this->len = newlen;
...
TString TString::operator-(const TString& s) const
{
	...
	if (pos) {
		...
		strncpy(s3.content, this->content, beghalf);
		strncpy(s3.content + beghalf, pos + s.len, s3.len - beghalf);
		s3.content[s3.len] = '\0';
		return s3;
	}
	else
...
TString TString::operator-(const char* cs) const
{
	...
	if (pos) {
		...
		int beghalf = pos - this->content;
		strncpy(s3.content, this->content, beghalf);
		strncpy(s3.content + beghalf, pos + slen, s3.len - beghalf);
		s3.content[s3.len] = '\0';
		return s3;
	}
	else
...
TString TString::operator-(const char c) const
{
	...
	if (pos) {
		...
		int beghalf = pos - this->content;
		strncpy(s3.content, this->content, beghalf);
		strncpy(s3.content + beghalf, pos + 1, s3.len - beghalf);
		s3.content[s3.len] = '\0';
		return s3;
	}
	else
...
TString& TString::operator-=(const TString& s)
{
	...
	if (pos) {
		...
		int beghalf = pos - this->content;
		strncpy(newContent, this->content, beghalf);
		strncpy(newContent + beghalf, pos + s.len, this->len - beghalf);
		delete[] this->content;
		this->content = newContent;
		this->content[this->len] = '\0';
	}
...
TString& TString::operator-=(const char* cs)
{
	...
	if (pos) {
		...
		int beghalf = pos - this->content;
		strncpy(newContent, this->content, beghalf);
		strncpy(newContent + beghalf, pos + slen, this->len - beghalf);
		delete[] this->content;
		this->content = newContent;
		this->content[this->len] = '\0';
	}
...
TString& TString::operator-=(const char c)
{
	...
	if (pos) {
		...
		int beghalf = pos - this->content;
		strncpy(newContent, this->content, beghalf);
		strncpy(newContent + beghalf, pos + 1, this->len - beghalf);
		delete[] this->content;
		this->content = newContent;
		this->content[this->len] = '\0';
	}
...
TString TString::operator*(const int num) const
{
	...
	for (int i = 0; i < num; i++) 
		strncpy(ret.content + i * this->len, this->content, this->len);
...
TString& TString::operator*=(const int num)
{
	...
	char* newContent = new(nothrow) char[newlen + 1];
	for (int i = 0; i < num; i++) 
		strncpy(newContent + i * this->len, this->content, this->len);
	newContent[newlen] = '\0';
	delete[] this->content;
	this->content = newContent;
	this->len = newlen;
